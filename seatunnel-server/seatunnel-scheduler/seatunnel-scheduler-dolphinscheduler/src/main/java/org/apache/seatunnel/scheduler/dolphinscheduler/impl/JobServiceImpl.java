...
public class JobServiceImpl implements IJobService {
    ...
    public long submitJob(JobDto dto) {
        ...
        iDolphinschedulerService.updateProcessDefinitionState(
                processDefinition.getCode(), processDefinition.getName(), RELEASE_STATE_ONLINE);
    ...
    public void offlineJob(JobDto dto) {
        iDolphinschedulerService.updateProcessDefinitionState(
                dto.getJobId(), dto.getJobName(), RELEASE_STATE_OFFLINE);
    }
    ...
    public PageData<JobSimpleInfoDto> list(JobListDto dto) {
        ...
        final List<JobSimpleInfoDto> data =
                processPageData.getData().stream()
                        .map(
                                p ->
                                        JobSimpleInfoDto.builder()
                                                .jobId(p.getCode())
                                                .jobName(p.getName())
                                                .jobStatus(p.getReleaseState())
                                                .publish(
                                                        ReleaseStateEnum.parse(p.getReleaseState())
                                                                .isPublish())
    ...
    public InstanceDto execute(ExecuteDto dto) {
        ...
        if (dto.getExecuteTypeEnum() == ExecuteTypeEnum.RERUN) {
            iDolphinschedulerService.execute(
                    dto.getJobDto().getJobId(),
                    org.apache.seatunnel.scheduler.dolphinscheduler.ExecuteTypeEnum.REPEAT_RUNNING);
            return null;
        }
        ...
        final StartProcessDefinitionDto startProcessDefinitionDto =
                StartProcessDefinitionDto.builder()
                        ...
                        .dryRun(DRY_RUN)
                        .scheduleTime(
                                DateUtils.format(startTime, DEFAULT_DATETIME_FORMAT)
                                        .concat(COMMA)
                                        .concat(DateUtils.format(endTime, DEFAULT_DATETIME_FORMAT)))
                        .execType(execType)
        ...
        if (Objects.nonNull(processDefinition)) {
            ...
            for (int i = 0; i < maxWaitingTimes; i++) {
                ...
                final InstanceListDto instanceListDto =
                ...
                final PageData<InstanceDto> instancePageData =
                        iInstanceService.list(instanceListDto);
            ...
            CompletableFuture.runAsync(
                            () -> {
                                ...
                                iDolphinschedulerService.updateProcessDefinitionState(
                                        code, name, RELEASE_STATE_OFFLINE);
                            })
                    .whenComplete(
                            (_return, e) -> {
                                if (Objects.nonNull(e)) {
                                    log.error(
                                            "clear temporary process definition failed, process definition code is [{}], name is [{}]",
                                            code,
                                            name,
                                            e);
                                }
    ...
    private ProcessDefinitionDto getProcessDefinitionDto(JobDto dto) {
        final TaskDescriptionDto taskDescriptionDto =
                TaskDescriptionDto.builder()
                        .name(dto.getJobName())
        ...
        final UpdateProcessDefinitionDto processDto =
                UpdateProcessDefinitionDto.builder()
                        .name(dto.getJobName())
