...
public class DolphinschedulerServiceImpl implements IDolphinschedulerService, InitializingBean {
    ...
    public ProcessDefinitionDto createOrUpdateProcessDefinition(UpdateProcessDefinitionDto dto) {
        ...
        List<TaskDefinitionDto> taskDefinitionJson =
                Collections.singletonList(
                        buildTaskDefinitionJson(taskCode, dto.getTaskDescriptionDto()));
        List<TaskRelationDto> taskRelationJson =
                buildTaskRelationJson(taskCode, dto.getTaskDescriptionDto());
        ...
        if (Objects.nonNull(dto.getProcessDefinitionCode())) {
            ...
            updateProcessDefinitionState(
                    dto.getProcessDefinitionCode(), dto.getName(), RELEASE_STATE_OFFLINE);
        }
        ...
        final Map<String, String> paramMap =
                createParamMap(
                        ...
                        this.objectToString(taskDefinitionJson),
                        TASK_RELATION_JSON,
                        this.objectToString(taskRelationJson),
                        TENANT_CODE,
                        defaultTenantName,
    ...
    public PageData<ProcessDefinitionDto> listProcessDefinition(ListProcessDefinitionDto dto) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(QUERY_LIST_PAGING, defaultProjectCode)))
        ...
        final List<Map<String, Object>> processDefinitionList =
                (List<Map<String, Object>>) map.get(DATA_TOTAL_LIST);

        if (CollectionUtils.isEmpty(processDefinitionList)) {
            return new PageData<>(total, Collections.emptyList());
        }
        final List<ProcessDefinitionDto> data =
                processDefinitionList.stream()
                        .map(m -> this.mapToPojo(m, ProcessDefinitionDto.class))
    ...
    public ProcessDefinitionDto fetchProcessDefinitionByName(String processDefinitionName) {
        ...
        final Map<String, Object> map =
                (Map<String, Object>) MapUtils.getMap(result, DATA).get(PROCESS_DEFINITION);
    ...
    public void startProcessDefinition(StartProcessDefinitionDto dto) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(START_PROCESS_INSTANCE, defaultProjectCode)))
    ...
    public List<SchedulerDto> listSchedule(long processDefinitionCode) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(
                                                QUERY_SCHEDULE_LIST_PAGING, defaultProjectCode)))
                        .withMethod(Connection.Method.GET)
                        .withData(
                                createParamMap(
        ...
        final List<Map<String, Object>> scheduleList =
                (List<Map<String, Object>>) MapUtils.getMap(result, DATA).get(DATA_TOTAL_LIST);
        if (CollectionUtils.isEmpty(scheduleList)) {
            return Collections.emptyList();
        }
        return scheduleList.stream()
                .map(m -> this.mapToPojo(m, SchedulerDto.class))
                .collect(Collectors.toList());
    }
        ...
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(
                                                SCHEDULE_ONLINE, defaultProjectCode, scheduleId)))
                        .withMethod(Connection.Method.POST)
        ...
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(
                                                SCHEDULE_OFFLINE, defaultProjectCode, scheduleId)))
                        .withMethod(Connection.Method.POST)
                        .withData(createParamMap(SCHEDULE_ID, scheduleId))
                        .withToken(TOKEN, token)
                        .execute(Map.class);
    ...
    public ResourceDto createOrUpdateScriptContent(String resourceName, String content) {
        ...
        if (Objects.isNull(parentResourceDto)) {
            throw new SeatunnelException(
                    SeatunnelErrorEnum.NO_MATCHED_SCRIPT_SAVE_DIR, defaultScriptDir);
        }
        final ResourceDto dto =
                getResourceDto(fullName.concat(DEFAULT_FILE_SUFFIX), RESOURCE_TYPE_FILE);
        if (Objects.isNull(dto)) {
            final OnlineCreateResourceDto createDto =
                    OnlineCreateResourceDto.builder()
    ...
    public PageData<TaskInstanceDto> listTaskInstance(ListTaskInstanceDto dto) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(QUERY_TASK_LIST_PAGING, defaultProjectCode)))
                        .withMethod(Connection.Method.GET)
                        .withData(
                                createParamMap(
        ...
        checkResult(result, false);
        final Map map = MapUtils.getMap(result, DATA);
        final List<Map<String, Object>> taskInstanceList =
                (List<Map<String, Object>>) map.get(DATA_TOTAL_LIST);
        final int total = MapUtils.getIntValue(map, DATA_TOTAL);
        ...
        final List<TaskInstanceDto> data =
                taskInstanceList.stream()
                        .map(m -> this.mapToPojo(m, TaskInstanceDto.class))
                        .collect(Collectors.toList());
        return new PageData<>(total, data);
    }
    ...
    public PageData<ProcessInstanceDto> listProcessInstance(ListProcessInstanceDto dto) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(PROCESS_INSTANCE_LIST, defaultProjectCode)))
                        .withMethod(Connection.Method.GET)
                        .withData(
                                createParamMap(
                                        SEARCH_VAL,
        ...
        final Map map = MapUtils.getMap(result, DATA);
        final List<Map<String, Object>> processInstanceList =
                (List<Map<String, Object>>) map.get(DATA_TOTAL_LIST);
        final int total = MapUtils.getIntValue(map, DATA_TOTAL);
        ...
        final List<ProcessInstanceDto> data =
                processInstanceList.stream()
                        .map(m -> this.mapToPojo(m, ProcessInstanceDto.class))
                        .collect(Collectors.toList());
        return new PageData<>(total, data);
    }
    ...
    public InstanceLogDto queryInstanceLog(long instanceId) {
                ...
                HttpUtils.builder()
                        ...
                        .withData(
                                createParamMap(
                                        ...
                                        instanceId,
                                        LOG_SKIP_LINE_NUM,
                                        LOG_SKIP_LINE_NUM_DEFAULT,
                                        LOG_LIMIT_NUM,
                                        LOG_LIMIT_NUM_DEFAULT))
                        .withMethod(Connection.Method.GET)
                        .withToken(TOKEN, token)
                        .execute(Map.class);
        checkResult(result, false);
        ...
        return InstanceLogDto.builder().logContent(logContent).build();
    }
    ...
    private ProjectDto queryProjectCodeByName(String projectName) throws IOException {
        ...
        final List<Map<String, Object>> projectList =
                (List<Map<String, Object>>) MapUtils.getMap(result, DATA).get(DATA_TOTAL_LIST);
        final ProjectDto projectDto =
                projectList.stream()
                        .map(m -> this.mapToPojo(m, ProjectDto.class))
    ...
    private ResourceDto getResourceDto(String fullName, String fileType) {
        final Map result =
                HttpUtils.builder()
                        .withUrl(
                                apiPrefix.concat(
                                        String.format(QUERY_RESOURCE, RESOURCE_ID_DEFAULT)))
                        .withMethod(Connection.Method.GET)
                        .withData(
                                createParamMap(
                                        FULL_NAME, fullName, RESOURCE_TYPE, RESOURCE_TYPE_FILE))
                        .withToken(TOKEN, token)
                        .execute(Map.class);
