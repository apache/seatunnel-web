...
import { fieldMap } from './use-task-config'
...
export function formatParams(data: INodeData): {
  ...
  if (
    data.taskType &&
    ['SHELL', 'PYTHON', 'SEATUNNEL', 'NEXT_LOOP', 'SPARK'].includes(
      data.taskType
    )
  ) {
    taskParams.rawScript = data.script
    taskParams.fieldStr = fieldMap.rawScript
  }

  if (data.taskType === 'SHELL' || data.taskType === 'PYTHON') {
    taskParams.dataSourceList = data.dataSourceList?.filter(
      (item) => item.type && item.datasource
    )
  }
  ...
  if (data.taskType === 'SQOOP') {
    ...
    if (data.isCustomTask) {
      // customShell
      taskParams.customShell = data.script
      taskParams.fieldStr = fieldMap.SQOOP
    } else {
          ...
          sourceParams = {
            ...
            srcColumns:
              data.srcQueryType === '1' || data.srcColumnType === '0'
                ? ''
                : data.srcColumns,
            srcQuerySql: data.srcQueryType === '0' ? '' : data.script,
  ...
  if (data.taskType === 'SQL') {
    ...
    taskParams.sql = data.script
    taskParams.fieldStr = fieldMap.SQL
  ...
  if (data.taskType === 'PROCEDURE') {
    taskParams.type = data.type
    taskParams.datasource = data.datasource
    taskParams.method = data.script
    taskParams.fieldStr = fieldMap.PROCEDURE
  }
  ...
  if (data.taskType === 'WHALE_SEATUNNEL') {
    taskParams.taskId = data.taskId
    taskParams.breakContinue = data.breakContinue
  }
  ...
  if (data.taskType === 'DATAX') {
    ...
    if (taskParams.customConfig === 0) {
      ...
      taskParams.sql = data.script
      taskParams.fieldStr = fieldMap.DATAX_IF
      ...
      taskParams.json = data.script
      taskParams.fieldStr = fieldMap.DATAX_ELSE
  ...
  if (data.taskType === 'DEPENDENT') {
    ...
    taskParams.otherParams = {
      dependStrategy: data.dependStrategy
    }
  }
  ...
  if (data.taskType === 'EMR') {
    taskParams.type = data.type
    taskParams.programType = data.programType
    if (data.programType === 'RUN_JOB_FLOW') {
      taskParams.jobFlowDefineJson = data.script
      taskParams.fieldStr = fieldMap.RUN_JOB_FLOW_IF
    } else {
      taskParams.stepsDefineJson = data.script
      taskParams.fieldStr = fieldMap.RUN_JOB_FLOW_ELSE
    }
  }
  ...
  if (data.taskType === 'OPENMLDB') {
    ...
    taskParams.sql = data.script
    taskParams.fieldStr = fieldMap.OPENMLDB
  }
  if (data.taskType === 'SAGEMAKER') {
    taskParams.sagemakerRequestJson = data.script
    taskParams.fieldStr = fieldMap.SAGEMAKER
  }
  if (data.taskType === 'PYTORCH') {
    taskParams.script = data.script
    taskParams.fieldStr = fieldMap.PYTORCH
    ...
    taskParams.showOtherParams = data.showOtherParams
  }
  if (data.taskType === 'PIGEON') {
    taskParams.targetJobName = data.targetJobName
  }
  if (data.taskType === 'SURVEIL') {
    taskParams.interval = data.interval
    taskParams.scanType = data.scanType
    if (taskParams.scanType === 1) {
      taskParams.filePath = data.filePath
    }
    if (taskParams.scanType === 4) {
      taskParams.filePath = data.filePath
    }
    if (taskParams.scanType === 2) {
      taskParams.type = data.type
      taskParams.datasource = data.datasource
      taskParams.sql = data.script
      taskParams.fieldStr = fieldMap.SURVEIL_TYPE2
      taskParams.sqlType = '2'
    }
    if (taskParams.scanType === 3) {
      taskParams.topic = data.topic
      taskParams.bootstrapServers = data.bootstrapServers
      taskParams.groupId = data.groupId
      taskParams.keyDeserializer = data.keyDeserializer
      taskParams.valueDeserializer = data.valueDeserializer
      taskParams.offsetTime = data.offsetTime
    }
  }
  if (data.taskType === 'NEXT_LOOP') {
    taskParams.customConfig = data.customConfig ? 1 : 0
    taskParams.programType = data.programType
    taskParams.cardCode = data.cardCode
  }
  if (data.taskType === 'INFORMATICA') {
    taskParams.informaticaFolderName = data.informaticaFolderName
    taskParams.informaticaWorkflowName = data.informaticaWorkflowName
  }
  if (data.taskType === 'HIVECLI') {
    taskParams.hiveCliTaskExecutionType = data.hiveCliTaskExecutionType
    taskParams.hiveSqlScript = data.script
    taskParams.fieldStr = fieldMap.HIVECLI
  ...
  if (data.taskType === 'CHUNJUN') {
    taskParams.customConfig = data.customConfig ? 1 : 0
    taskParams.json = data.script
    taskParams.fieldStr = fieldMap.CHUNJUN
    taskParams.deployMode = data.deployMode
    taskParams.others = data.others
  }

  if (data.taskType === 'JAR') {
    taskParams.programType = data.programType
    taskParams.options = data.options
    taskParams.cpResIds = data.cpResIds
    taskParams.mainClass = data.mainClass
    if (data.mainJar) {
      taskParams.mainJar = { id: data.mainJar }
    }
  ...
  const params = {
    ...
    taskDefinitionJsonObj: {
      ...
      taskParams: {
        ...
        ideContentField: {
          resourceId: data?.resourceId || null,
          field: taskParams.fieldStr || null
        }
      },
...
export function formatModel(data: ITaskData) {
  ...
  if (data.taskParams?.sourceParams) {
    ...
    params.script = sourceParams.srcQuerySql
  ...
  if (data.taskParams?.hiveSqlScript) {
    params.script = data.taskParams?.hiveSqlScript
  }
  ...
  if (data.taskParams?.jobFlowDefineJson) {
    params.script = data.taskParams.jobFlowDefineJson
  }

  if (data.taskParams?.sagemakerRequestJson) {
    params.script = data.taskParams.sagemakerRequestJson
  }
  ...
  if (data.taskParams?.otherParams?.dependStrategy) {
    params.dependStrategy = data.taskParams.otherParams.dependStrategy
  }
  if (data.taskParams?.rawScript) {
    params.script = data.taskParams.rawScript
  }
  if (data.taskParams?.method) {
    params.script = data.taskParams.method
  }
  if (data.taskParams?.sql) {
    params.script = data.taskParams.sql
  }
  if (data.taskType === 'CHUNJUN' && data.taskParams?.json) {
    params.script = data.taskParams.json
  }

  if (data.taskParams?.remoteConnection) {
    params.open = data.taskParams.remoteConnection.open
    params.datasourceType = data.taskParams.remoteConnection.datasourceType
    params.datasourceId = data.taskParams.remoteConnection.datasourceId
  }
