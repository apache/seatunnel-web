...
export function usePytorch(model: { [field: string]: any }): IJsonItem[] {
  ...
  const setFlag = () => {
    model.showCreateEnvironment = model.isCreateEnvironment && model.showOtherParams
  ...
  const resetSpan = () => {
    isCreateEnvironmentSpan.value = model.showOtherParams ? 12 : 0
    pythonPathSpan.value = model.showOtherParams ? 24 : 0
    pythonEnvToolSpan.value = model.showCreateEnvironment ? 12 : 0
    pythonCommandSpan.value =
      ~model.showCreateEnvironment & model.showOtherParams ? 12 : 0
    requirementsSpan.value = model.showCreateEnvironment ? 24 : 0
    condaPythonVersionSpan.value = model.showCreateConda ? 24 : 0
  }

  watch(
    () => [model.isCreateEnvironment, model.pythonEnvTool, model.showOtherParams],
    () => {
      setFlag()
      resetSpan()
    }
