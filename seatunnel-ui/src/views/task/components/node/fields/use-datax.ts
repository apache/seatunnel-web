...
export function useDataX(
  ...
  watch(
    ...
    () => {
      ...
      const handlers = model.customConfig
        ? [{ key: 'script', name: t('project.node.datax_json_template') }]
        : [{ key: 'script', name: t('project.node.sql_statement') }]
      if (updateValue) {
        updateValue(
          {
            handlers,
            language: model.customConfig ? 'json' : 'sql',
            script: ''
          },
          'batch'
        )
  ...
  return [
    ...
    ...useDatasource({
      ...
      typeName: 'project.node.datax_target_datasource_type',
      sourceName: 'project.node.datax_target_database'
    }),
    {
      type: 'input',
      field: 'targetTable',
