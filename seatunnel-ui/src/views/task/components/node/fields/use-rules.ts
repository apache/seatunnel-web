...
export function useRules(
  ...
  const formatResponseJson = (
    ...
    if (model[item.field] !== void 0 && !reset) {
  ...
  const onFieldChange = async (
    ...
    if (field === 'src_connector_type' && typeof value === 'string') {
    ...
    if (field === 'target_connector_type' && typeof value === 'string') {
      const result = await getDatasourceOptionsById(value)
    ...
    if (field === 'writer_connector_type' && typeof value === 'string') {
      const result = await getDatasourceOptionsById(value)
    ...
    if (field === 'src_datasource_id' && typeof value === 'string') {
    ...
    if (field === 'target_datasource_id' && typeof value === 'string') {
      const result = await getDatasourceTablesById(value)
  ...
  return [
    {
      ...
      props: {
        loading: ruleLoading,
        onUpdateValue: (ruleId: number) => {
          getRuleById(ruleId, true)
        }
      },
      options: rules
    },
    {
      ...
      validate: {
        ...
        validator(validate: any, value: number) {
          if (model.comparison_type === '1' && !value && value !== 0) {
            return new Error(t('project.node.fix_value_tips'))
          }
        }
      },
      span: fixValueSpan
